...
def clean_expired():
    ...
    for row in rows:
        room_id = row["id"]
        files = con.execute("SELECT stored_name FROM files WHERE room_id=?", (room_id,)).fetchall()
        for file in files:
            path = UPLOADS / file["stored_name"]
            if path.exists():
                path.unlink()
        con.execute("DELETE FROM files WHERE room_id=?", (room_id,))
...
def create():
    ...
    con.execute("INSERT INTO rooms VALUES (?, ?, ?, ?)",
                (room_id, password, time.time(), time.time() + hours * 3600))
    con.commit()
    con.close()
...
def join():
    ...
    password = request.form.get("password", "").strip()
    con = db()
    room_data = con.execute("SELECT * FROM rooms WHERE id=?", (room_id,)).fetchone()
    con.close()
    if not room_data:
        return render_template("home.html", error="Room not found. Please check the Room ID.")
...
def room(room_id):
    ...
    if room_data["password"]:
        ...
        if session.get(session_key) != room_data["password"]:
            if request.method == "POST" and request.form.get("password") == room_data["password"]:
    ...
    expires = datetime.fromtimestamp(room_data["expires"]).strftime("%d %b %Y, %I:%M %p")
    return render_template("room.html", room_id=room_id, share_url=share_url,
                           expires=expires, qr_filename=qr_filename)

@app.get("/api/room/<room_id>/files")
def list_files(room_id):
    room_id = room_id.upper()
    con = db()
    rows = con.execute("""SELECT id, original_name, size, created FROM files
                          WHERE room_id=? ORDER BY id DESC""", (room_id,)).fetchall()
    con.close()
...
def upload(room_id):
    ...
    if not room_data:
        con.close()
        abort(404)
    if room_data["password"] and session.get(f"room_{room_id}") != room_data["password"]:
        con.close()
        abort(403)
    saved = []
    for file in request.files.getlist("files"):
        ...
        con.execute("""INSERT INTO files
            ...
            VALUES (?, ?, ?, ?, ?)""",
            (room_id, stored_name, original_name, path.stat().st_size, time.time()))
...
def download(file_id):
    con = db()
    file_data = con.execute("SELECT * FROM files WHERE id=?", (file_id,)).fetchone()
    con.close()
    if not file_data:
        abort(404)
    return send_from_directory(UPLOADS, file_data["stored_name"],
                               as_attachment=True,
...
def delete(file_id):
    ...
    if not file_data:
        con.close()
        abort(404)
    room_data = con.execute("SELECT * FROM rooms WHERE id=?",
                            (file_data["room_id"],)).fetchone()
    if room_data["password"] and session.get(f"room_{room_data['id']}") != room_data["password"]:
        con.close()
        abort(403)
